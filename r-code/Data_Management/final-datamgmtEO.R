...
#Ran: 11/3/2021 by Elizabeth Oys 
...
seedbank.colfax.data <- read_excel(path = here("data", "Updated_GH_Data", "Colfax_GH_Results_MASTER.xlsx"), sheet = "Sheet1") %>%
  mutate(Site = "Colfax")
seedbank.enrec.data <- read_excel(path = here("data", "Updated_GH_Data", "ENREC_GH_Results_MASTER.xlsx"), sheet = "RESULTS") %>%
  mutate(Site = "ENREC")
seedbank.greeley.data <- read_excel(path = here("data", "Updated_GH_Data", "Greeley_GH_Results_MASTER.xlsx"), sheet = "Results") %>%
  mutate(Site = "Greeley")
seedbank.howard.data <- read_excel(path = here("data", "Updated_GH_Data", "Howard_GH_Results_MASTER.xlsx"), sheet = "RESULTS") %>%
  mutate(Site = "Howard")
seedbank.merrick.data <- read_excel(path = here("data", "Updated_GH_Data", "Merrick_GH_Results_MASTER.xlsx"), sheet = "RESULTS") %>%
  mutate(Site = "Merrick")
seedbank.scal.data <- read_excel(path = here("data", "Updated_GH_Data", "SCAL_GH_Results_MASTER.xlsx"), sheet = "RESULTS") %>%
  mutate(Site = "SCAL")
...
speciesList <- names(seedbank.data)[c(4:9, 17:68)]
speciesList
...
sumseedbank.data <- seedbank.data %>%
  ...
  select(Site, Treatment, Rep, names(seedbank.data)[c(6, 8:68)]) %>%
  mutate(TOTAL_SPECIES = rowSums(.[,c(8:65)] > 0)) %>%
  select(Site, Treatment, Rep, TOTAL_SPECIES, names(seedbank.data)[c(6, 8:68)])

# EXPORT COMBINED SEEDBANK DATA
#write.csv(sumseedbank.data, file = here("data", "updatedseedbank-data-sum.csv"), row.names = F, na = "")
...
write.csv(combined.data, file = here("data", "updated-combined-dataEO.csv"), row.names = F, na = "")
