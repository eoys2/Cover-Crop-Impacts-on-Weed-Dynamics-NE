...
field.data <- rbind(field.early.data, field.late.data) %>%
  ...
  select(Site, Treatment, Rep, Sample, Sample_Date, Season, Current_crop, Previous_crop, Crop_Stage, 
         CoverCrop_Biomass, Biomass_Pigweed, Biomass_Grass, Biomass_Broadleaf, Biomass_VolunteerCC, Biomass_Total, 
         Weed_Density, Pigweed_Density, Grass_Density, Broadleaf_Density, VolunteerCC_Density, Moisture, Temperature) %>%
...
# write.csv(field.data, file = here("data", "field-data-original.csv"), row.names = F, na = "")

# AVERAGE OVER SAMPLES
avgfield.data <- field.data %>%
  group_by(Site, Treatment, Rep, Sample_Date, Season, Current_crop, Previous_crop, Crop_Stage) %>%
  summarise(avgCoverCrop_Biomass   = mean(CoverCrop_Biomass),
            avgBiomass_Pigweed     = mean(Biomass_Pigweed),
            avgBiomass_Grass       = mean(Biomass_Grass),
            avgBiomass_Broadleaf   = mean(Biomass_Broadleaf),
            avgBiomass_VolunteerCC = mean(Biomass_VolunteerCC),
            ...
            sumVolunteerCC_Density = sum(VolunteerCC_Density),
...
# EXPORT COMBINED FIELD DATA
# write.csv(avgfield.data, file = here("data", "field-data-aggregate.csv"), row.names = F, na = "")
...
combined.data <- full_join(avgfield.data, sumseedbank.data, by = joinCols) %>%
  ...
  select(Site, Treatment, Rep, Sample_Date, Season, Current_crop, Previous_crop, Crop_Stage, 
         avgCoverCrop_Biomass, avgBiomass_Total, sumWeed_Density, totSeeds_Weeds, Total_Species,
         ...
         avgBiomass_VolunteerCC, sumVolunteerCC_Density, 
         totSeeds_Otherbroad,
...
# write.csv(combined.data, file = here("data", "combined-data.csv"), row.names = F, na = "")
